...
import { 
  ...
  MapPinned, CloudSun, Megaphone, User, Warehouse, 
  TrendingUp, Calculator, FlaskConical, Sprout, Tractor, 
  Beaker, Apple, CalendarDays, BookOpen, Activity, Users,
  Quote, Star, Heart
} from 'lucide-react';
import RotatingLogo from '../components/RotatingLogo';
import DigitalClock from '../components/DigitalClock';

// DATA POOL: Farmer Greetings & Multilingual Wisdom
const GREETINGS = [
...
export default function Dashboard() {
  ...
  return (
    <div className={`p-4 md:p-10 space-y-6 text-right min-h-screen transition-all duration-500 ${theme === 'high-contrast' ? 'bg-black text-yellow-400' : 'bg-[#050505] text-white'}`} dir="rtl">
      ...
      {/* 2. IQBAL & HADITH HEADER (Nastaleeq Font Applied) */}
      ...
      {/* 3. RESTORED & IMPROVED TILES GRID */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 pb-20">
        <ToolTile 
        ...
        />
        <ToolTile 
          onClick={() => handleToolClick('/market', false)} 
          icon={BarChart3} label="منڈی ریٹ" sub="MARKET" 
          bg="bg-amber-600" 
        />
        <ToolTile 
        ...
        />
        <ToolTile 
          onClick={() => handleToolClick('/machinery', false)} 
          icon={Tractor} label="مشینری" sub="MACHINERY" 
          bg="bg-lime-600" 
        />
        <ToolTile 
          onClick={() => handleToolClick('/news', false)} 
          icon={BookOpen} label="زرعی خبریں" sub="NEWS" 
          bg="bg-sky-600" 
        />
      </div>
    </div>
