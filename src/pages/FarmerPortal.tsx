...
import { 
  ...
  Trash2, RefreshCw, Plus, Volume2, AlertTriangle, 
  LayoutDashboard, LogOut, User, Sprout, CloudRain, Sun, CloudLightning 
} from 'lucide-react';
...
export default function FarmerPortal() {
  ...
  // 🗓️ DATE STATE (For Auto-Schedule)
  const [startDate, setStartDate] = useState(new Date().toISOString().split('T')[0]);

  // 🍎 APPLE STAGES DATA (With Weather Warnings)
  const APPLE_STAGES = [
    ...
    { id: 4, name: "Bloom (Flower)", ur: "پھول (بلوم)", spray: "🚫 NO SPRAY", days: 35, warning: "Do NOT Spray! Bees Active", risk: "Critical" },
  ...
  const getDateAfterDays = (start: string, days: number) => {
      ...
      return result.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }); // e.g. "12 Feb"
  };

  // DEFAULT DATA
  const DEFAULT_TASKS = [
    { id: 1, title: "Spray Fungicide", titleUr: "پھپھوندی مار دوا کا سپرے", date: "Today", type: "Urgent", status: "pending", warning: "Rain Expected!" },
    { id: 2, title: "Apply Urea", titleUr: "یوریا کھاد ڈالیں", date: "Tomorrow", type: "Normal", status: "pending" }
  ];
  ...
  useEffect(() => {
    const initPortal = async () => {
      try {
        ...
        if (puter?.auth) setUser(await puter.auth.getUser());

        let data = DEFAULT_TASKS;
        if (puter?.kv) {
          const saved = await puter.kv.get('fck_my_tasks');
          if (saved) data = JSON.parse(saved);
        }
        setTasks(data);
      } catch (err) { setTasks(DEFAULT_TASKS); } 
  ...
  const saveToDb = async (newTasks: any[]) => {
    ...
    try {
      const puter = (window as any).puter;
      if (puter?.kv) await puter.kv.set('fck_my_tasks', JSON.stringify(newTasks));
    } catch (err) { console.error(err); }
  };

  const markDone = (id: number) => saveToDb(tasks.map(t => t.id === id ? { ...t, status: 'completed' } : t));
  const deleteTask = (id: number) => { if(confirm("Delete?")) saveToDb(tasks.filter(t => t.id !== id)); };

  // ADD TASK LOGIC
  const addCustomTask = (e: any) => {
    e.preventDefault();
    const title = e.target.title.value;
    const titleUr = e.target.titleUr.value || title;
    const isUrgent = e.target.urgent.checked;

    const newTask = {
      id: Date.now(),
      title, titleUr, date: "Today", 
      type: isUrgent ? "Urgent" : "Normal", 
      status: "pending"
    };
    saveToDb([newTask, ...tasks]);
    setShowModal(false);
  };
  ...
  const addStageTask = (stage: any) => {
      ...
      const newTask = {
          ...
          warning: stage.warning // Saving warning to task
      };
      saveToDb([newTask, ...tasks]);
      setShowModal(false);
  };
  ...
  const addFullSchedule = () => {
      if(!confirm("Create full 3-month schedule based on this start date?")) return;
    ...
    <div className="min-h-screen bg-[#020408] text-white p-6 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
            <button onClick={() => setShowModal(true)} className="p-3 bg-emerald-600 rounded-xl hover:bg-emerald-500 shadow-lg shadow-emerald-900/40 flex items-center gap-2 text-xs font-black uppercase tracking-widest animate-pulse">
                <Plus size={18} /> <span className="hidden md:inline">Add Task / Schedule</span>
            </button>
            {user && (
      ...
      <div className="flex gap-2 p-1 bg-white/5 rounded-2xl mb-6">
          <button onClick={() => setActiveTab('pending')} className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-white text-black shadow-lg' : 'text-white/40 hover:text-white'}`}>Pending</button>
          <button onClick={() => setActiveTab('completed')} className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'completed' ? 'bg-white text-black shadow-lg' : 'text-white/40 hover:text-white'}`}>Completed</button>
      </div>
             ...
             visibleTasks.map(task => (
                 <div key={task.id} className={`p-5 rounded-[2rem] border flex flex-col gap-4 group transition-all duration-300 ${task.type === 'Urgent' ? 'bg-rose-500/5 border-rose-500/50 shadow-lg shadow-rose-900/20' : 'bg-[#0a0c10] border-white/5 hover:border-white/20'}`}>
                     ...
                     <div className="flex items-center justify-between">
                         <div className="flex items-center gap-4">
                             ...
                             <div>
                                 ...
                                 {task.details && <p className="text-white/40 text-xs mt-1 italic">{task.details}</p>}
                             </div>
                         </div>
                         <div className="text-right">
                             <div className="flex flex-col items-end">
                                 <span className="text-xs font-black text-white">{task.date}</span>
                                 <span className="text-[9px] text-white/30 uppercase">Due Date</span>
                             </div>
                         </div>
                     </div>

                     {/* 🌧️ WEATHER WARNING STRIP */}
                     {task.warning && (
                     ...
                     <div className="flex gap-2 justify-end border-t border-white/5 pt-3 mt-1">
                         {activeTab === 'pending' && (
                             <button onClick={() => markDone(task.id)} className="px-4 py-2 bg-emerald-500/10 text-emerald-500 rounded-lg hover:bg-emerald-500 hover:text-white transition-all text-xs font-bold uppercase flex items-center gap-2"><CheckCircle2 size={14}/> Mark Done</button>
                         )}
                         <button onClick={() => deleteTask(task.id)} className="px-4 py-2 bg-rose-500/10 text-rose-500 rounded-lg hover:bg-rose-500 hover:text-white transition-all text-xs font-bold uppercase flex items-center gap-2"><Trash2 size={14}/> Remove</button>
                     </div>
                 </div>
             ))
          )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-4">
          <div className="bg-[#121212] w-full max-w-lg rounded-[2.5rem] border border-white/10 shadow-2xl animate-in slide-in-from-bottom overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 overflow-y-auto flex-1">
               ...
               {modalTab === 'custom' && (
                 <form onSubmit={addCustomTask} className="space-y-4">
                    <div>
                        <label className="text-xs font-bold text-white/40 uppercase ml-2">Title (Eng)</label>
                        <input name="title" required placeholder="Spray..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white outline-none focus:border-emerald-500" />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-white/40 uppercase ml-2">Title (Urdu)</label>
                        <input name="titleUr" placeholder="سپرے..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white font-urdu text-right outline-none focus:border-emerald-500" />
                    </div>
                    <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl">
                        <input type="checkbox" name="urgent" id="urgent" className="w-5 h-5 accent-rose-500" />
                        <label htmlFor="urgent" className="font-bold text-sm">Mark as Urgent</label>
                    </div>
                    <button type="submit" className="w-full bg-emerald-600 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-500">Add Task</button>
                 </form>
               )}

               {/* 2. STAGES WITH DATES & WEATHER */}
               {modalTab === 'stages' && (
                 <div className="space-y-4">
                    ...
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                        <label className="block text-xs font-bold text-emerald-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                            <Calendar size={14}/> Start Date (Dormant Stage)
                        </label>
                        <input 
                          type="date" 
                          value={startDate}
                          onChange={(e) => setStartDate(e.target.value)}
                          className="w-full bg-black border border-white/20 rounded-xl p-3 text-white outline-none focus:border-emerald-500"
                        />
                    </div>

                    <button onClick={addFullSchedule} className="w-full py-3 bg-white/10 hover:bg-emerald-600/20 text-emerald-400 hover:text-white rounded-xl text-xs font-black uppercase tracking-widest border border-dashed border-emerald-500/30 transition-all">
                        + Add Full 3-Month Schedule
                    </button>

                    <p className="text-center text-white/30 text-[10px] uppercase tracking-widest">Or select single stage</p>

                    {/* Stages List */}
                    <div className="space-y-2">
                        {APPLE_STAGES.map(stage => {
                            const calculatedDate = getDateAfterDays(startDate, stage.days);
                            return (
                                <button 
                                  key={stage.id} 
                                  onClick={() => addStageTask(stage)}
                                  className="w-full bg-white/5 hover:bg-white/10 p-4 rounded-2xl border border-white/5 flex justify-between items-center text-left group transition-all"
                                >
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded text-white/60 font-mono">{calculatedDate}</span>
                                            {stage.risk === 'Critical' && <span className="text-[9px] bg-rose-500 text-white px-2 py-0.5 rounded uppercase font-bold">Critical</span>}
                                        </div>
                                        ...
                                        {/* Warning Preview */}
                                        <div className="flex items-center gap-1 mt-2 text-rose-300/70 text-[10px]">
                                            <CloudRain size={10} />
            ...
            <button onClick={() => setShowModal(false)} className="w-full py-5 bg-black/40 text-white/40 text-xs font-bold uppercase tracking-widest hover:text-white border-t border-white/5 shrink-0">
                Close
            </button>
          </div>
        </div>
