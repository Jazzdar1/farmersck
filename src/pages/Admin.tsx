...
import { 
  ...
  Megaphone, BookOpen, Video, FileText, Check, AlertTriangle, Loader2, Globe 
} from 'lucide-react';
...
export default function Admin() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!puter?.auth) { navigate('/admin-login'); return; }
  ...
  const saveAllData = async () => {
    ...
    // 2. PUBLIC BROADCAST (This makes alerts visible everywhere)
    try {
        await puter.fs.mkdir('~/www').catch(() => {});
        await puter.fs.write('~/www/alerts.json', JSON.stringify(newsList));
    } catch(e) { console.log("Public write failed"); }

    setLoading(false);
    alert("✅ Changes Saved & Broadcasted Live!");
  };
    ...
    <div className="min-h-screen bg-[#020408] text-white p-6 font-sans" dir="rtl">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 bg-[#0a0c10] p-6 rounded-[2rem] border border-white/5 shadow-2xl gap-4">
        ...
        <div className="flex gap-2">
            <button onClick={saveAllData} className="px-6 py-3 bg-emerald-600 rounded-xl font-bold text-xs uppercase hover:bg-emerald-500 flex items-center gap-2 shadow-lg shadow-emerald-900/40"><Globe size={16}/> Publish Live</button>
          ...
          <div className="animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <button onClick={() => { setEditingTile({ id: '', label: '', sub: '', link: '/', icon: 'Sprout', gradient: '', shadow: '', isActive: true } as any); setIsNewTile(true); }} className="border-2 border-dashed border-white/10 rounded-[2.5rem] flex flex-col items-center justify-center min-h-[180px] hover:bg-white/5 hover:border-emerald-500/50 transition-all text-white/30 hover:text-emerald-500 gap-2">
                      <Plus size={40}/><span className="text-xs font-bold uppercase tracking-widest">Add New Tile</span>
                  </button>
              </div>
          </div>
      )}
              ...
              {newsList.map(news => (
                  <div key={news.id} className="bg-[#0a0c10] p-6 rounded-[2rem] border border-white/5 flex justify-between items-center">
                      <div className="flex items-center gap-4">
                          ...
                          <div><h4 className="font-bold text-lg text-white mb-1">{news.title}</h4><p className="text-white/50 text-sm font-urdu">{news.message}</p></div>
                      </div>
                      <div className="flex gap-2">
                  ...
                  {knowledgeList.map(item => (
                      <div key={item.id} className="bg-[#0a0c10] p-6 rounded-[2rem] border border-white/5">
                          <div className="flex justify-between items-start mb-4">
                              <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest ${item.category === 'Video' ? 'bg-red-500/20 text-red-500' : 'bg-emerald-500/20 text-emerald-500'}`}>{item.category}</span>
                              <div className="flex gap-2">
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
             <div className="bg-[#1a1d24] w-full max-w-lg rounded-[2.5rem] p-8 border border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
                 ...
                 <div className="space-y-4">
                     ...
                     <div className="grid grid-cols-6 gap-2">{Object.keys(THEMES).map(t => <button key={t} onClick={() => setSelectedTheme(t)} className={`h-10 rounded-xl bg-gradient-to-br ${THEMES[t].gradient}`}/>)}</div>
                 </div>
                 <div className="flex gap-3 mt-8">
                     <button onClick={handleTileSave} className="flex-1 bg-emerald-600 py-4 rounded-xl font-bold">Save</button>
                     <button onClick={() => setEditingTile(null)} className="flex-1 bg-white/5 py-4 rounded-xl font-bold">Cancel</button>
                 </div>
             </div>
         </div>
      )}
      ...
      {editingNews && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
             <div className="bg-[#1a1d24] w-full max-w-lg rounded-[2.5rem] p-8 border border-white/10">
                 ...
                 <div className="space-y-4">
                     <input value={editingNews.title} onChange={e => setEditingNews({...editingNews, title: e.target.value})} className="w-full bg-black/40 p-4 rounded-xl text-white" placeholder="Title" />
                     <textarea value={editingNews.message} onChange={e => setEditingNews({...editingNews, message: e.target.value})} className="w-full bg-black/40 p-4 rounded-xl text-white font-urdu text-right" placeholder="Message" />
                     <div className="flex gap-4">
                         <label className="flex items-center gap-2"><input type="radio" name="ntype" checked={editingNews.type === 'Info'} onChange={() => setEditingNews({...editingNews, type: 'Info'})}/> Info</label>
                         <label className="flex items-center gap-2"><input type="radio" name="ntype" checked={editingNews.type === 'Warning'} onChange={() => setEditingNews({...editingNews, type: 'Warning'})}/> Warning</label>
                     </div>
                 </div>
                 <div className="flex gap-3 mt-8">
                     <button onClick={handleNewsSave} className="flex-1 bg-blue-600 py-4 rounded-xl font-bold">Save</button>
      ...
      {editingKnow && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
             <div className="bg-[#1a1d24] w-full max-w-lg rounded-[2.5rem] p-8 border border-white/10">
                 ...
                 <div className="space-y-4">
                     <input value={editingKnow.title} onChange={e => setEditingKnow({...editingKnow, title: e.target.value})} className="w-full bg-black/40 p-4 rounded-xl text-white" placeholder="Title" />
                     <textarea value={editingKnow.desc} onChange={e => setEditingKnow({...editingKnow, desc: e.target.value})} className="w-full bg-black/40 p-4 rounded-xl text-white" placeholder="Description" />
                     <input value={editingKnow.link} onChange={e => setEditingKnow({...editingKnow, link: e.target.value})} className="w-full bg-black/40 p-4 rounded-xl text-white" placeholder="URL" />
                     <div className="flex gap-4">
                         <label className="flex items-center gap-2"><input type="radio" name="ktype" checked={editingKnow.category === 'Article'} onChange={() => setEditingKnow({...editingKnow, category: 'Article'})}/> Article</label>
                         <label className="flex items-center gap-2"><input type="radio" name="ktype" checked={editingKnow.category === 'Video'} onChange={() => setEditingKnow({...editingKnow, category: 'Video'})}/> Video</label>
                     </div>
                 </div>
                 <div className="flex gap-3 mt-8">
                     <button onClick={handleKnowSave} className="flex-1 bg-purple-600 py-4 rounded-xl font-bold">Save</button>
                     <button onClick={() => setEditingKnow(null)} className="flex-1 bg-white/5 py-4 rounded-xl font-bold">Cancel</button>
                 </div>
             </div>
         </div>
      )}
