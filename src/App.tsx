...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-white flex flex-col lg:flex-row font-sans">
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 bg-[#050608] border-r border-white/5 transition-transform lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} overflow-y-auto`}>
        <div className="p-5 flex flex-col min-h-full">
          <div className="flex items-center gap-4 mb-8 shrink-0 bg-[#0a0c10] p-4 rounded-3xl border border-white/5 shadow-inner">
            <div className="w-14 h-14 bg-gradient-to-br from-emerald-400 to-green-700 rounded-2xl flex items-center justify-center font-black text-white text-2xl shadow-lg shadow-emerald-500/20 border border-emerald-300/30">
              <Sprout size={24} />
            </div>
            <div><h1 className="font-black text-xl tracking-tighter uppercase text-white leading-none">FC KASHMIR</h1><p className="text-[12px] text-emerald-500 font-bold mt-1">کشمیر کسان</p></div>
          </div>
...
function NavLink({ to, icon: Icon, en, ur, active, onClick, gradient, shadow }: any) {
  return (
    <Link to={to} onClick={onClick} className={`relative flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 mb-3 group border ${active ? `bg-gradient-to-br ${gradient} border-white/20 shadow-lg ${shadow}` : 'bg-transparent border-transparent hover:bg-white/5 hover:border-white/10'}`}>
